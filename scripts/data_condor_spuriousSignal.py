...
for channelNames in allChannelNames:
  ...
  for sigmean in sigmeans:
    for sigwidth in sigwidths:
      cmd = "python -b "+cdir + "/scripts/data_spuriousSignal.py" +  \
                           " --pdFitName=" + pdFitName + \
                           " --fitName=" + fitName + \
      ...
      runfile = "batch/runSpurious_" + str(sigmean) + "_" + str(sigwidth) +  "_" + pdFitName + "_" + fitName + "_" + overallName + "_" + signalfile + "_%d"%(channelNames[0]) + ".sh"
      fr=open(runfile,'w')
      fr.write('#!/bin/sh\n')
