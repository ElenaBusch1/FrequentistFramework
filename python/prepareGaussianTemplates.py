...
def getGaussianFunction(signalMass, alpha, inputFile, inputFileWidth, histName, syst, maxX = 11000, sigma=0.1, pruneThreshold = 0.1):
  ...
  isPruned = True
  if syst.find("JER") >=0 or 100.*(muValUp - signalMass)/signalMass > pruneThreshold:
    isPruned = False
...
def prepareGaussianTemplate(indir = "/afs/cern.ch/work/j/jroloff/nixon/systematics/", histName = "h2_resonance_jet_m4j_alpha", doSysts = True, mY = 6000, alpha = 5, outfile = "systematicsGaus", maxX=11000, sigma = 0.1, pruneThreshold = 0.1, systematicNameFile = "uncertaintySets/systematics.txt"):
